...
public:
    ...
    ImageEncoder(skel_tree_t* forest, bool into) {
        int i = 0;
        for (int x = -1; x <= 1; ++x) {
            for (int y = -1; y <= 1; ++y) {
                if (x == 0 && y == 0) continue;
                for (int r = -2; r <= 2; ++r) {
                    // printf("%d: %d - %d - %d\n", i++, x, y, r);
                    point_list[make_tuple(x, y, r)] = 0; 
                    point_list_[make_tuple(x, y, r)] = i;//Wang. change 0 to i to use it for second encoding method;
                    points[i++] = make_tuple(x, y, r);
                    // predictive_hist[i++] = (int*)(calloc(40, sizeof(int)));
                }
            }
        ...
        populate_encoding_histogram(forest);
        if (into)
            cout<<"Got here!!!"<<endl;
        // set up the predictor
        // int j = 0;
...
protected:
    ...
    std::map<std::tuple<int, int, int>, int> point_list, point_list_;
...
*/
    TraditionalEncoder(skel_tree_t* forest) : ImageEncoder(forest, 1) {
        std::cout << "Traditional" << endl;
    };
    ...
    string encode_difference(int dx, int dy, int dr) {
        ...
        if (small_point) {
            ...
            encoded_point.append(encoded_dx + encoded_dy + encoded_dr);
            /*
           //Wang. change to another encoding method, only need to store 40 possible combination.
           
           encoded_point.append("0");
           encoded_point.append("1");
           auto encoded = bitset<6>(point_list_[make_tuple(dx, dy, dr)]).to_string();
           encoded_point.append(encoded);
           */
           
           // cout << "point: " << encoded_point << endl;
