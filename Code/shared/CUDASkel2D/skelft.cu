...
__global__ void kernelSkel(float* output, bool* fg, short xm, short ym, 
						   short xM, short yM, short size, float threshold,float IMPthreshold, float length)	
...
{																			//Output:	output: binary thresholded skeleton
	...
	if (tx>xm && ty>ym && tx<xM && ty<yM) 
	{
	 ...
	 float Saliency = Imp / dt; //salience
	  ...
	  if (fg[id] && Saliency>=threshold && Imp>IMPthreshold) output[Id] = Saliency;	//wang.The original one is fg[id].				//By filling only 1-values, we reduce memory access somehow (writing to output[] is expensive)
		//if (fg[id] && Imp>threshold) output[Id] = Imp;	//wang.The original one is fg[id].				//By filling only 1-values, we reduce memory access somehow (writing to output[] is expensive)
...
void skelft2DDT(float* outputDT, 									  							//Compute DT (into pbaTextures[2]) from resident FT (in pbaTextures[1])
...
void skelft2DSkeleton(float* outputSkel, bool* fg,float length, float threshold,float Impthreshold,	//Compute thresholded skeleton (into pbaTextures[3]) from resident FT (in pbaTextures[1])
					  short xm,short ym,short xM,short yM)						
{																				//length:     boundary length
    ...
    kernelSkel<<< grid, block >>>((float*)pbaTextures[3], fg, xm, ym, xM-1, yM-1, pbaTexSize, threshold,Impthreshold, length);
